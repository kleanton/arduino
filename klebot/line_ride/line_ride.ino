...
//                   НОМЕРА ВЫВОДОВ:
const uint8_t        pinSensorL   = A2;                             // Вывод к которому подключен датчик находящийся слева         (по направлению движения)
const uint8_t        pinSensorC   = A3;                             // Вывод к которому подключен датчик находящийся по центру     (по направлению движения)
const uint8_t        pinSensorR   = A6;                             // Вывод к которому подключен датчик находящийся справа        (по направлению движения)
const uint8_t        pinSens_TRIG = 13;                              // Вывод к которому подключен датчик расстояния HC_SR04        (вывод обозначенный на датчике как TRIG)
...
const uint16_t       valSensor1   = 63;                            // Показание датчика находящегося на линии                     (указывается для конкретной трассы)
const uint16_t       valSensor0   = 174;                            // Показание датчика находящегося вне линии                    (указывается для конкретной трассы)
...
void forward_left(int set_speed_L, int set_speed_R ) 
{ 
  // блокировка вращения левых колес. 
  digitalWrite(motor_L1, set_speed_L); 
  digitalWrite(motor_L2, set_speed_R); 
  // правые колеса вращаются. 
  digitalWrite(motor_Rl, set_speed_R); 
  digitalWrite(motor_R2, set_speed_L); 
} 

// Поворот направо с блокировкой правых колес, 
void forward_right(int set_speed_L, int set_speed_R) 
{ 
...
//  Определяем действия в соответствии с текущим положением датчиков
  switch(valSensor){ //            Сохраняем время:    Меняем флаг ШИМ:   Меняем флаг поворота:  Меняем флаг остановки:
    ...
    Serial.begin(9600); while(!Serial){}                            // Инициируем передачу данных по последовательному порту       (на скорости 9600 бит/сек)
    Serial.println(analogRead(pinSens_TRIG));                         // Выводим показания центрального датчика линии                (для указания значений константам valSensor0 и valSensor1)
//  Устанавливаем задержку и обновляем время совершения последней операции
    delay(2000);
...
//      Выводим ШИМ
       forward_left(arrSpeed[1],arrSpeed[0]);
       forward_right  (arrSpeed[0],arrSpeed[1]);
